...
- 채점 Shell은 CloudShell에서 실행하며 계정 확인, 명시 리전, 이름/태그 기반 리소스 탐색, AWS CLI query, jq/curl/SSM, PASS/FAIL과 점수 근거를 사용한다. 원본 예시의 상태 변경 명령은 안전성 검토 없이 복사하지 않는다.
...
- 예제처럼 명시적인 리소스 이름과 설정값 줄을 사용한다. Region은 하나만 선택하고 문서·스크립트·배포파일 전체에서 일치시킨다.
...
[채점 Shell script 품질]
- CloudShell에서 실행한다. Bastion EC2를 사용하지 않으며, 불가피하면 이유를 과제와 기준표에 명시한다.
